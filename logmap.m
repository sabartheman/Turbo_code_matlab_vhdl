...
function [LLR,a1,a2,a3,a4,b1,b2,b3,b4,g1,g2,g3,g4,g5,g6,g7,g8,Le] = hdl_logmap(r,ap,Lc)
...
for i = 2:length(a)                                         % compute alpha. eq.(6.29) and (6.36)
    ...
    a(3,i) = max(a1,a2) + log(1+exp(-abs(a1-a2)));
        
    a1 = g1(7,i-1) + a(2,i-1); a2 = g1(8,i-1) + a(4,i-1);
    a(4,i) = max(a1,a2) + log(1+exp(-abs(a1-a2)));
end
...
for i = length(b)-1:-1:1                                    % compute beta. eq.(6.29) and (6.36)
    ...
    b(3,i) = max(b1,b2) + log(1+exp(-abs(b1-b2)));
        
    b1 = g(7,i) + b(3,i+1); b2 = g(8,i) + b(4,i+1);
    b(4,i) = max(b1,b2) + log(1+exp(-abs(b1-b2)));
end

for i=1:nn                                                  % compute LLR. eq.(6.31) and (6.36)                                                   % 
    spa = a(1,i) + g(1,i) + b(1,i+1); spb = a(3,i) + g(4,i) + b(2,i+1); 
    sp1 = max(spa,spb) + log(1+exp(-abs(spa-spb)));
        
    spa = a(2,i) + g(6,i) + b(4,i+1); spb = a(4,i) + g(7,i) + b(3,i+1);
    sp2 = max(spa,spb) + log(1+exp(-abs(spa-spb)));
        
    sp(i) = max(sp1,sp2) + log(1+exp(-abs(sp1-sp2)));
        
    sma = a(1,i) + g(2,i) + b(2,i+1); smb = a(3,i) + g(3,i) + b(1,i+1);
    sm1 = max(sma,smb) + log(1+exp(-abs(sma-smb)));
        
    sma = a(2,i) + g(5,i) + b(3,i+1); smb = a(4,i) + g(8,i) + b(4,i+1);
    sm2 = max(sma,smb) + log(1+exp(-abs(sma-smb)));
        
    sm(i) = max(sm1,sm2) + log(1+exp(-abs(sm1-sm2)));
end
